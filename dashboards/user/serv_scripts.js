...
//event lock
var event_lock_req = async function (eid,lock,socket){
    var result = await global.process_sql_call("CALL event_set_lock("+eid+","+lock+",false)");
    if(result == null){
        socket.emit('c_ref',"Unkonwn Error");
        return;
    }
...
//Event reset loans
var reset_loans = async function(eid,socket){
    var result = await global.process_sql_call("CALL event_reset_loans ("+eid+",false)");
    if(result == null){
        socket.emit('c_ref',"Unkonwn Error");
        return;
    }
...
//Event reset loans
var reset_ludo = async function(eid,socket){
    var result = await global.process_sql_call("CALL ludo_recreate_table ("+eid+",false)");
    if(result == null){
        socket.emit('c_ref',"Unkonwn Error");
        return;
    }
...
//Event empty ludo
var empty_ludo = async function(eid,socket){
    var result = await global.process_sql_call("CALL event_empty_ludo("+eid+",false)");
    if(result == null){
        socket.emit('c_ref',"Unkonwn Error");
        return;
    }
...
var recover_event_info=async function(e_id,socket){
    var result=await global.process_sql_call("CALL event_get_info("+e_id+")",null);
    if(result == null){
        socket.emit('c_err',"Unkonwn Error");
        return;
    }
    ...
    socket.emit("eeif_show",result._Link);
    return;
}

var set_event_info=async function(e_id,e_site,socket){
    var result = await global.process_sql_call('CALL event_set_info('+e_id+',"'+e_site+'",false)');
    standard_reply(result,socket);
    return;
}

//<!-- Editing USER Information-->
var set_user_pwd = async function(u_id,npwd,socket){
    var result=await global.process_sql_call("CALL user_set_password("+u_id+",'"+npwd+"',false)",null);
     if(result == null){
        socket.emit('c_err',"Unkonwn Error");
        return;
    }
    ...
    socket.emit("c_msg","Password cambiata con successo");
    return;
}

//GAME DELETE
var delete_game = async function (gid,socket){
    var result=await global.process_sql_call("CALL collection_del_game("+gid+",false)",null);
     standard_reply(result,socket);
    return;
}

//add game to event
var eadd_game = async function(title,owner,eid,socket){
    var result=await global.process_sql_call("CALL collection_add_event_game('"+title+"','"+owner+"',"+eid+",false)",null);
     standard_reply(result,socket);
    return;
}
...
//Event loan manage
        ...
        //event lock
        socket.on("event_lock_req",(eid,lock) =>{
             console.log('received event_lock_req('+eid+')');
             event_lock_req(eid,lock,socket);
        });


        //Event reset loans
        socket.on("reset_loans",(eid)=>{
            console.log('received reset_loans('+eid+')');
             reset_loans(eid,socket);
        });

        //Event  reset_ludo
        socket.on("reset_ludo",(eid)=>{
            console.log('received reset_ludo('+eid+')');
             reset_ludo(eid,socket);
        });

        //Event  reset_ludo
        socket.on("empty_ludo",(eid)=>{
            console.log('received empty_ludo('+eid+')');
             empty_ludo(eid,socket);
        });

        //<!-- Editing EVENT Information-->
        socket.on("eeif_show",(eid)=>{
            console.log('received eeif_show('+eid+')');
            recover_event_info(eid,socket);
        });

        socket.on("eeif_request",(eid,site)=>{
            console.log('received eeif_request('+eid+','+site+')');
            set_event_info(eid,site,socket);
        });

        //<!-- Editing USER Information-->
        socket.on("eupw_request",(uid,pwd)=>{
            console.log('received eupw_request('+uid+','+pwd+')');
            set_user_pwd(uid,pwd,socket);
        });

        //GAME DELETE
        socket.on("game_del",(gid)=>{
            console.log('received game_del('+gid+')');
            delete_game(gid,socket);
        ...
        //ADD GAME TO EVENT
        socket.on("eadd_game",(title,owner,eid)=>{
            console.log('received eadd_game()');
