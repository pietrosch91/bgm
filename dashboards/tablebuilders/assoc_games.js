...
function parse_line(line_in){
    ...
    line_out.mio = (line_in.owner === current_assoc);
...
function create_line(data_in,use_bgg,viewonly){
    ...
    if(p_data.mio){
        ...
        if(p_data.prestato_da){
            html_table_temp+='<td>'+current_assoc+'</td>';
        }
        else{
            html_table_temp+='<td></td>';
        }
...
var build_table= async function (wrapname,reqdata,sql_func,addlast,use_bgg=false,viewonly=false){
    current_assoc=reqdata.assoc_name;
    var result = await global.process_sql_call("CALL "+sql_func+"("+reqdata.assoc_id+")",null,1);
