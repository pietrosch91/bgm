...
async function create_admin_subtable(aid){
    var html_temp='<table style="borders:none">';
    var result=await global.process_sql_call("CALL access_get_admins("+aid+")",null,1);
    var nadmins=result.length;
    if(nadmins>3) return "ERROR";
    for(let i=0;i<nadmins;i++){
        html_temp+='<tr style="borders:none"><td style="borders:none">'+result[i]._name+'</td><td style="borders:none"><button onclick="remove_admin('+result[i]._id+','+aid+')">Rimuovi</button></td></tr>';
    }
    if(nadmins<3){
        html_temp+='<tr style="borders:none"><td style="borders:none" colspan="2"><button onclick="add_admin('+aid+')">Aggiungi</button></td></tr>';
    }
    html_temp+="</table>";
    return html_temp;
}
...
async function create_line(data_in){
    ...
    var id=data_in.ass_ID;
    var name=data_in.ass_Name;
    var code=data_in.ass_Code;
    var ena=data_in.ass_Enabled;
    ...
    if(ena){
        html_table_temp+='<td><button onclick="set_assoc_enabled('+id+',0)">Disattiva</button></td>';
    }
    else{
        html_table_temp+='<td><button onclick="set_assoc_enabled('+id+',1)">Attiva</button></td>';
    }
    ...
    html_table_temp+='<td>'+adm_table+'</td>';
        ...
        // </td>';
    html_table_temp+='<td><button onclick="delete_assoc('+id+')">Elimina Associazione</button></td>';
    html_table_temp+="</tr>";
...
//
    ...
    //Last line
    html_table_out+='<tr><td/><td><button onclick="create_assoc()">Aggiungi Associazione</button></td><td/><td/><td/></tr>';
...
var build_table= async function (wrapname){
    var result = await global.process_sql_call("CALL association_get_view_admin()",null,1);
